...
Column layout:
    ...
    C: Rating
    D onward: every other column from either source. A batter row leaves
...
BATTER_COLUMNS = [
    "RBI_EV", "TB_EV", "TB_EV_Estimated", "Boost", "Slot3", "Slot4", "Slot5",
...
def main():
    ...
    combined = []
    for row in batters:
        out = {"Player": row.get("Player", ""), "Type": "Batter", "Rating": row.get("Rating", "")}
    ...
    for row in pitchers:
        out = {"Player": row.get("Player", ""), "Type": "Pitcher", "Rating": row.get("Rating", "")}
        for col in BATTER_COLUMNS:
            out[col] = ""
        for col in PITCHER_COLUMNS:
            out[col] = row.get(col, "")
    ...
    fieldnames = ["Player", "Type", "Rating"] + BATTER_COLUMNS + PITCHER_COLUMNS
