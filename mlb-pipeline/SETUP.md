...
3. Before clicking Deploy, click **Advanced settings** and add secrets:
   ```
   ODDS_API_KEYS = "key1,key2,key3,key4,key5,key6,key7"
   ADMIN_PASSWORD = "pick-something-only-you-know"
   ```
   `ADMIN_PASSWORD` is new - it's what gates the "Run Pipeline Now" button so
   other people who open the app can view results but can't trigger a run
   (each run burns your odds-api quota, so this matters). Anyone without the
   password only sees the results table.
4. Click **Deploy**. You'll get a URL like `https://your-app-name.streamlit.app`.

**Also upload the `.streamlit` folder** (contains `config.toml`, just a theme
file) the same way you had to upload `.github` - it starts with a dot, so
drag-and-drop from Finder/Explorer will likely skip it invisibly. If the
app's colors look like default Streamlit orange instead of the intended
neutral theme, this folder didn't make it in - use "Create new file" on
GitHub's website and type `.streamlit/config.toml` as the filename (same
trick as the workflow file) if drag-and-drop doesn't pick it up.
