...
def _best_pitcher_group(pitchers, n, excluded_teams):
    """Top n pitchers by value, preferring ones not facing a stacked team
    where possible. Ranked using AVERAGE_MULTIPLIER as a stand-in since we
    don't yet know which slot each will actually land in (that's decided
    later, globally, in score_combo) - a higher-Rating pitcher is a better
    pick in any slot, so this ranking is a safe proxy for candidate
    selection even though it isn't the final scoring formula. Returns [] if
    n == 0, None if not enough pitchers exist at all."""
    if n == 0:
        return []
    if len(pitchers) < n:
        return None

    eligible = [p for p in pitchers if p.get("Opponent") not in excluded_teams]
    pool = eligible if len(eligible) >= n else pitchers
...
def build_best_lineup(batters, pitchers, correlation_pct, risk_pct):
    ...
    for num_pitchers in range(0, LINEUP_SIZE + 1):
        ...
        pitcher_group = _best_pitcher_group(pitchers, num_pitchers, batter_teams)
