...
PITCHER_SLOT_MULTIPLIER = 1.0  # neutral - see apply_odds_and_rating for why
...
def apply_odds_and_rating(row, player_name, odds_lookup):
    ...
    # Boost/PitcherSlot mirror the batter Slot1-5 pattern (Rating * (Boost + multiplier)),
    # but a pitcher only fills one roster spot rather than five ordered batting
    # slots, so there's just one multiplier. 1.0 = neutral (no positional
    # weighting baked in, unlike batting-order slots) - the website lets each
    # visitor try their own boost live, per-session, same as batters.
    row["Boost"] = 0.0
    row["PitcherSlot"] = round(row["Rating"] * (row["Boost"] + PITCHER_SLOT_MULTIPLIER), 4)
    return row
...
def main():
    ...
    fieldnames = [
        ...
        "Strikeouts", "Walks", "ER", "Outs", "Hits", "Rating", "Confidence", "Boost", "PitcherSlot", "Odds_Used_For",
    ]
