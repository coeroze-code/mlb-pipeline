...
values to build a correlation/stacking-aware 5-man batting lineup plus a
starting pitcher. All of this is entirely per-browser-session: Streamlit
...
if os.path.exists(CSV_PATH):
    ...
    if editing_enabled:
        ...
        for n, mult in SLOT_MULTIPLIERS.items():
            ...
            if col in working.columns:
                working.loc[is_batter, col] = (working.loc[is_batter, "Rating"] * (boost_numeric[is_batter] + mult)).round(4)

        if "PitcherSlot" in working.columns:
            working.loc[is_pitcher, "PitcherSlot"] = (
                working.loc[is_pitcher, "Rating"] * (boost_numeric[is_pitcher] + PITCHER_SLOT_MULTIPLIER)
            ).round(4)
    ...
    if "Opponent" in df.columns:
        pitcher_pool = _boosted_pool("Pitcher", ["Player", "Team", "Opponent", "Rating", "Boost"])
    ...
    if len(batter_pool) + len(pitcher_pool) < 5:
        ...
        st.caption(
            "Picks the best 5 players from batters and pitchers combined - there's no fixed split, "
            "the optimizer decides how many of each based on value, same-team stacking correlation, "
            "and lineup risk. Private to your session - nobody else sees your lineup or slider settings."
        )
