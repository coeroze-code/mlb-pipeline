...
values to build a correlation/stacking-aware 5-man lineup. Both are entirely
per-browser-session: Streamlit gives every visitor their own isolated
session_state on the server, so one person's edits, sliders, and built
lineup are never visible to, or overwrite, anyone else's - and nothing here
...
if os.path.exists(CSV_PATH):
    ...
    editing_enabled = type_filter == "Batter" and "Boost" in shown.columns and "Player" in shown.columns
    if editing_enabled:
        ...
        if type_filter == "All" and "Boost" in df.columns:
            st.caption("Switch to the Batter filter to edit Boost - it's not editable in this combined view.")
    ...
    # ----------------------------------------------------------------------
    batter_pool = df[df["Type"] == "Batter"][["Player", "Team", "Rating", "Boost", "Confidence"]].copy()
    if "boosts" in st.session_state:
        batter_pool["Boost"] = batter_pool.apply(
            lambda r: st.session_state["boosts"].get(r["Player"], r["Boost"]), axis=1
        )
    for col in ["Rating", "Boost", "Confidence"]:
        batter_pool[col] = pd.to_numeric(batter_pool[col], errors="coerce")
    batter_pool = batter_pool.dropna(subset=["Player", "Team", "Rating"])

    st.divider()
    st.subheader("Lineup Builder")
    if len(batter_pool) < 5:
        st.caption("Not enough batters with data yet to build a lineup.")
    else:
        st.caption(
            "Builds the best 5-man batting lineup using your current Boost values above, factoring in "
            "same-team stacking correlation and lineup risk. Private to your session - nobody else sees "
            "your lineup or slider settings."
        )
        ...
        if st.button("Build Lineup", type="primary"):
            st.session_state["lineup_result"] = lineup_builder.build_lineup(
                batter_pool.to_dict("records"), correlation_pct, risk_pct
            )
        ...
        if result:
            lineup_rows = [
                {
                    ...
                    "Slot Value": round(p["Rating"] * (p["Boost"] + lineup_builder.SLOT_MULTIPLIERS[i]), 3),
                }
            ...
            lm2.metric("Base Value", result["base_value"])
            lm3.metric("Stack Bonus", f"+{result['stack_bonus']}")
            lm4.metric("Est. Std Dev", result["stddev"])
else:
